...
$releaseFolder = Join-Path $projectRoot "release\SB-Launcher-3.5.0"
...
try {
  ...
  dotnet publish apps/native/SBLauncher.Native.csproj -c Release -r win-x64 --self-contained true -p:Version=3.5.0 -p:AssemblyVersion=3.5.0.0 -p:FileVersion=3.5.0.0 -o $releaseFolder
  ...
  $buildInfo = @{ version="3.5.0"; channel="stable"; buildId=(Get-Date -Format "yyyyMMddHHmmss"); builtAt=(Get-Date).ToUniversalTime().ToString("o") } | ConvertTo-Json
