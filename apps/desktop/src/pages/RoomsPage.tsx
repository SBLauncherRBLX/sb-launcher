...
function Rooms(){
  ...
  useEffect(()=>{if(!room)return;let live=true;const timer=setInterval(()=>{void community<FriendRoom>("rooms.get",{code:room.code}).then(r=>{if(live){setRoom(r);setError("");}}).catch(e=>{if(live)setError(message(e));});},room.launchPlan?2000:15000);return()=>{live=false;clearInterval(timer);};},[room?.code,room?.launchPlan?.startedAt]);
  useEffect(()=>{
    ...
    void launchExperience({placeId:plan.placeId,gameInstanceId:plan.gameInstanceId,universeId:plan.universeId,name:plan.name,iconUrl:plan.iconUrl,serverType:"public"}).then(result=>{if(result)setStatus("The room is joining the same server.");}).catch(e=>{setError(message(e));setStatus("");});
  },[room?.launchPlan]);
  async function act(action:string,input:Record<string,unknown>={}){if(busy)return;setBusy(true);setError("");try{const result=await community<FriendRoom|{left:true}>(action,{...(room?{code:room.code}:{}),...input});if("left"in result){setRoom(null);await refresh();}else setRoom(result);}catch(e){setError(message(e));}finally{setBusy(false);}}
  async function launchRoom(){
    if(!room||!selected||room.ownerId!==userId)return;
    ...
    try{
      ...
      for(let page=0;page<8&&!candidate;page++){
        const result=await api.servers(selected.placeId,cursor);
        candidate=result.items.filter(server=>server.maxPlayers-server.playing>=room.members.length).sort((a,b)=>(b.maxPlayers-b.playing)-(a.maxPlayers-a.playing))[0]??null;
    ...
    }catch(e){setStatus("");setError(message(e));}finally{setBusy(false);}
  }
  ...
  return <div className="content-page"><header className="content-heading"><div><h2>{room?room.name:"Rooms"}</h2><p className="sb-muted">An invite-only queue, votes and ready checks for your group.</p></div>{room&&<Button variant="secondary" onClick={()=>{setRoom(null);void refresh();}}>My rooms</Button>}</header><ContentNotice error={error}/>{status&&<p role="status">{status}</p>}
    {!room?<><section className="sb-card content-panel"><h3>Start a room</h3><form className="content-toolbar" onSubmit={e=>{e.preventDefault();void act("rooms.create",{name});}}><input className="sb-input" aria-label="Room name" value={name} onChange={e=>setName(e.target.value)} maxLength={60}/><Button disabled={busy||!name.trim()}>Create room</Button></form><h3>Join with an invite code</h3><form className="content-toolbar" onSubmit={e=>{e.preventDefault();void act("rooms.join",{code:code.trim().toUpperCase()});}}><input className="sb-input" placeholder="12-character code" aria-label="Room invite code" maxLength={12} value={code} onChange={e=>setCode(e.target.value.toUpperCase())}/><Button disabled={busy||!/^[A-F0-9]{12}$/.test(code)}>Join room</Button></form></section><section className="sb-card content-panel"><div className="content-heading"><h3>My rooms</h3><Button variant="ghost" onClick={()=>void refresh()}>Refresh</Button></div>{rooms.map(r=><button className="room-list-item" key={r.code} onClick={()=>void act("rooms.get",{code:r.code})}><strong>{r.name}</strong><span>{r.members.length} members · {r.queue.length} games</span></button>)}{!rooms.length&&<p className="sb-muted">Create a room and send its code to your friends. Inactive rooms expire after 7 days.</p>}</section></>:<>
      <section className="sb-card content-panel"><div className="content-toolbar"><strong className="invite-code">{room.code}</strong><Button variant="secondary" onClick={()=>void navigator.clipboard.writeText(room.code).then(()=>setStatus("Invite code copied.")).catch(()=>setError("Could not copy. Select and copy the code above."))}>Copy invite</Button><Button disabled={busy} variant="ghost" onClick={()=>void act("rooms.leave")}>Leave room</Button>{room.ownerId===userId&&<Button disabled={busy} variant="ghost" onClick={()=>void act("rooms.close")}>Close room</Button>}</div><p className="sb-muted">Anyone with this code can join. Updates refresh every 15 seconds.</p><div className="room-members">{room.members.map(m=><div key={m.id}><span className={m.ready?"ready-dot is-ready":"ready-dot"}/><strong>{m.name}</strong><small>{m.id===room.ownerId?"Host · ":""}{m.ready?"Ready":"Not ready"}</small></div>)}</div><Button disabled={busy} onClick={()=>void act("rooms.ready",{ready:!me?.ready})}>{me?.ready?"Not ready":"I'm ready"}</Button></section>
      {selected&&<section className="sb-card content-panel room-selected"><h3>{selected.name}</h3><p>{room.members.filter(m=>m.ready).length}/{room.members.length} ready</p>{room.launchPlan?<p role="status">Shared server selected. Opening Roblox for all members…</p>:<Button disabled={busy||room.members.some(m=>!m.ready)} onClick={()=>void launchRoom()}>{busy?"Finding a server…":"Launch everyone together"}</Button>}<p className="sb-muted">The host starts the selected game after everyone is ready. SB Launcher finds a public server with enough free places and sends every member to the same server.</p></section>}
