...
function formatPatchDate(date: string) {
      ...
      <section className="about-patchnotes sb-card" aria-label="Patch notes">
        ...
        <article className="about-patch-block about-patch-block--major">
          <div className="about-patch-heading">
            ...
            <time className="sb-muted patch-date" dateTime={MAJOR_RELEASE.date}>{formatPatchDate(MAJOR_RELEASE.date)}</time>
          </div>
          <ul>
          ...
          {PATCH_NOTES.map((patch) => (
            <article key={patch.version} className="about-patch-block">
              <div className="about-patch-heading">
                ...
                <h4>{patch.title}</h4>
                <time className="sb-muted patch-date" dateTime={patch.date}>{patch.dateLabel ? `${patch.dateLabel} · ` : ""}{formatPatchDate(patch.date)}</time>
              </div>
              <ul>
